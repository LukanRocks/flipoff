...
# The frontend is a Vite app in the sibling web/ package. WEB_DIST is what gets
# served, so `pnpm -C web build` has to have run at least once. WEB_PUBLIC holds
# the source config.json, which is read for defaults whether or not a build exists.
WEB_ROOT = PROJECT_ROOT.parent / 'web'
...
_FRONTEND_CONFIG_PATH = WEB_PUBLIC / 'config.json'
...
RESERVED_BOARD_SLUGS = {'admin', 'api', 'assets', 'ws', 'screenshot.png', 'favicon.ico', 'index.html'}
...
async def control_handler(_: web.Request) -> web.Response:
    return web.FileResponse(WEB_DIST / 'control.html')


async def display_handler(_: web.Request) -> web.Response:
    return web.FileResponse(WEB_DIST / 'display.html')


async def config_json_handler(_: web.Request) -> web.Response:
    path = WEB_DIST / 'config.json'
    if path.exists():
...
async def admin_handler(_: web.Request) -> web.Response:
    return web.FileResponse(WEB_DIST / 'admin.html')


async def trailing_slash_redirect(request: web.Request) -> web.Response:
    # The bundle uses relative asset URLs (vite `base: './'`) so it can also be
    # deployed to a GitHub Pages sub-path. That only resolves correctly when the
    # page has no trailing slash, so normalise here instead of serving a page
    # whose scripts would 404.
    return web.HTTPMovedPermanently(request.path.rstrip('/') or '/')
...
async def screenshot_handler(_: web.Request) -> web.Response:
    return web.FileResponse(WEB_DIST / 'images' / 'screenshot.png')
...
def create_app(
    *,
    ...
    app.router.add_get('/admin/', trailing_slash_redirect)
    ...
    app.router.add_static('/assets', WEB_DIST / 'assets', follow_symlinks=False)
    app.router.add_get('/screenshot.png', screenshot_handler)
    app.router.add_get('/favicon.ico', favicon_handler)
    app.router.add_get('/{board_slug}', board_handler)
    app.router.add_get('/{board_slug}/', trailing_slash_redirect)
