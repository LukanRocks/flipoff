  ...
  async init() {
    ...
    try {
      ...
      const [mainBuffer, squeakBuffer, fartBuffer] = await Promise.all([
        ...
        this._loadExternalAudioBuffer(new URL('./audio/mixkit-rubber-duck-squeak-1014.wav', import.meta.url)),
        this._loadExternalAudioBuffer(new URL('./audio/farrrt.wav', import.meta.url)),
      ])
  ...
  _playJokeTransition(clusteredEvents, baseTime) {
    ...
    clusteredEvents.forEach((event, index) => {
      ...
      this._playSlice({
        ...
        panValue: Math.max(-0.42, Math.min(0.42, event.pan)),
        attackMs: 4,
        releaseMs: 70,
      })
  ...
  _extractTickSlices(buffer) {
    ...
    for (let i = 1; i < windows.length - 1; i++) {
      ...
      if (lastPeak && center - lastPeak.center < minSpacing) {
        if (cur.level > lastPeak.level) {
          lastPeak.center = center
          lastPeak.level = cur.level
        }
        continue
      }
    ...
    return [0.78, 0.93, 1.41, 1.68, 2.16, 2.82, 3.24].filter((o) => o < buffer.duration - 0.03).map((o) => ({ offset: o, duration: Math.min(0.085, buffer.duration - o) }))
  }
