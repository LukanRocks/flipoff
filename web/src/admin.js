...
async function loadAdminState({ successMessage = '', showSuccessMessage = false, boardSlug = currentBoardSlug } = {}) {
  try {
    ...
    const [config, screensPayload, messageState] = await Promise.all([configResponse.json(), screensResponse.json(), messageResponse.json()])
...
function applyBoardsPayload(payload, requestedBoardSlug = currentBoardSlug) {
  ...
  currentBoardSlug = availableBoards.some((board) => board.slug === requestedBoardSlug) ? requestedBoardSlug : fallbackSlug
...
function renderDashboardSummary() {
  ...
  if (overviewOverrideMeta) {
    overviewOverrideMeta.textContent = hasOverride ? `Override updated ${formatTimestamp(currentMessageState.updatedAt)}.` : 'Rotation is running without a temporary override.'
  }
...
function renderRotationOverview() {
  ...
  if (screenDrafts.length > visibleScreens.length) {
    rotationOverview.append(
      buildEmptyState(`${screenDrafts.length - visibleScreens.length} more ${pluralize('screen', screenDrafts.length - visibleScreens.length)} in the saved rotation.`),
    )
  }
}
...
function renderPluginHealth() {
  ...
  for (const [index, screen] of pluginScreens.entries()) {
    ...
    meta.textContent = screen.lastError ? screen.lastError : screen.lastRefreshedAt ? `Last refresh ${formatTimestamp(screen.lastRefreshedAt)}.` : 'Awaiting first refresh.'
...
function buildOverridePreviewLines() {
  ...
  const fallbackLine = screenDrafts[0] ? `Next: ${getScreenTitle(screenDrafts[0], 0)}` : 'Rotation idle'
...
function renderScreensList() {
  ...
  for (const [index, screen] of screenDrafts.entries()) {
    ...
    if (screen.type === 'plugin') {
      actions.append(buildScreenIconButton('Refresh plugin screen', 'refresh', 'refresh'))
    }

    actions.append(buildScreenIconButton('Edit screen', 'edit', 'edit'), buildScreenIconButton('Delete screen', 'delete', 'delete danger'))
...
function populatePluginSelect(selectedPluginId = '') {
  ...
  const nextValue = selectedPluginId && getPluginById(selectedPluginId) ? selectedPluginId : availablePlugins[0]?.id || ''
...
function renderPluginSchemaFields(settingsValues = null, designValues = null) {
  ...
  const commonSettingsValues = plugin.commonSettingsNamespace ? pluginCommonSettings[plugin.commonSettingsNamespace] || {} : {}
...
function handleDocumentClick(event) {
  ...
  if (event.target === pluginSelectTrigger || pluginSelectTrigger.contains(event.target) || pluginSelectMenu.contains(event.target)) {
    return
  }
...
function getScreenPreviewLines(screen) {
  const lines = Array.isArray(screen.previewLines) && screen.previewLines.length > 0 ? screen.previewLines : buildLocalPreviewLines(screen)
...
function buildLocalPreviewLines(screen) {
  ...
  return padLocalLines(['', title.slice(0, currentConfig.cols), detail.slice(0, currentConfig.cols)])
}
