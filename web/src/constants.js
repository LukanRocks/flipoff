...
//
// Resolved against this module's own URL rather than the page's, because the page
// can sit at a path we don't control: the backend serves boards at /{slug} and
// /{slug}/, and a GitHub Pages deploy can sit under /<repo>/. This module is always
// one directory below the deploy root — /src/constants.js in dev, /assets/*.js in a
// build (see `assetsDir` in vite.config.ts) — so '../config.json' lands on the root
// copy in every deployment.
let cfg = DEFAULTS
let _configLoaded = false
try {
  const configUrl = new URL(/* @vite-ignore */ `../config.json?_=${Date.now()}`, import.meta.url)
...
export const TOTAL_TRANSITION = MAX_VISIBLE_FLIPS * FLIP_STEP_DURATION + MAX_VISIBLE_FLIPS * FLIP_SETTLE_DURATION + 500
...
export const MATRIX_COLORS = ['#00FF41', '#00CC33', '#00FF41', '#00FF88', '#003B00', '#00FF41']
export const GRAYSCALE_COLORS = ['#888888', '#AAAAAA', '#666666', '#CCCCCC', '#444444', '#AAAAAA']
